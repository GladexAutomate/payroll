...
const Step = ({ index, label, children, done, locked, last }) => (
  <div className={`flex gap-3 ${locked ? 'opacity-50 pointer-events-none' : ''}`}>
    <div className="flex flex-col items-center pt-1">
      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${done ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground border border-border'}`}>
...
export default function ProposalWizard({ form, setForm, hierarchy, branchOptions, departmentOptions, roleOptions, teamOptions = [], onCreateTeam }) {
  ...
  return (
    <div>
      <Step index={1} label="1. Leader Name" done={!!form.leader_name}>
        <Input value={form.leader_name} onChange={e => set('leader_name', e.target.value)} placeholder="Enter leader name" />
      </Step>

      <Step index={2} label="2. Company" done={!!form.company_name} locked={!form.leader_name}>
        <Select
      ...
      <Step index={3} label="3. Branch" done={!!form.branch_name} locked={!form.company_name}>
        <Select
      ...
      <Step index={4} label="4. Department" done={!!form.department_name} locked={!form.branch_name}>
        <Select
      ...
      <Step index={5} label="5. Department Role" done={!!form.department_role} locked={!form.department_name}>
        <Select
      ...
      <Step index={7} label="7. Schedule Period" done={!!form.period_start && !!form.period_end} locked={!form.team_name} last>
        <PayPeriodPicker
