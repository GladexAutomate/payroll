...
export default function SetupCard({ title, subtitle, count, onManage, children, logoUrl }) {
  return (
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          {logoUrl ? (
            <img src={logoUrl} alt={title} className="w-10 h-10 rounded-xl object-contain bg-white border border-border shrink-0" />
          ) : (
