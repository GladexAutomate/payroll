...
Deno.serve(async (req) => {
  try {
    ...
    let records = await withRetry(() => base44.asServiceRole.entities.PayrollRecord.filter({ payroll_run_id }, '-created_date', 1000));
    while (records.length > 0) {
      await Promise.all(records.map(record => withRetry(() => base44.asServiceRole.entities.PayrollRecord.delete(record.id))));
      deletedRecords += records.length;
      await wait(250);
      records = await withRetry(() => base44.asServiceRole.entities.PayrollRecord.filter({ payroll_run_id }, '-created_date', 1000));
    }
